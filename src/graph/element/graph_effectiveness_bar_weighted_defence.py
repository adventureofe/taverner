...
def get_effectiveness_counts_defending_only(db_path):
    # Connect to the SQLite database
    connection = sqlite3.connect(db_path)
    ...
    for _, row in df.iterrows():
        ...
        if defender not in effectiveness_counts:
            effectiveness_counts[defender] = {}

        if effectiveness not in effectiveness_counts[defender]:
            effectiveness_counts[defender][effectiveness] = 0
        effectiveness_counts[defender][effectiveness] += 1
...
def create_stacked_bar_graph_defending_only(element_totals, effectiveness_order):
    # Prepare data for the graph
    categories = list(element_totals.keys())
    bar_segments = {eff_type: [] for eff_type in effectiveness_order}
    ...
    plt.xlabel("Elements")
    plt.ylabel("Weighted Totals (Defending Only)")
    plt.title("Weighted Effectiveness Stacked Bar Graph (Defending Only)")
    plt.legend(title="Effectiveness")
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.show()
...
if __name__ == "__main__":
    ...
    # Get attacking effectiveness data and create the attacking bar graph
    element_totals_attacking, effectiveness_order_attacking = get_effectiveness_counts_attacking_only(db_path)
    if element_totals_attacking:
        create_stacked_bar_graph_attacking_only(element_totals_attacking, effectiveness_order_attacking)

    # Get defending effectiveness data and create the defending bar graph
    element_totals_defending, effectiveness_order_defending = get_effectiveness_counts_defending_only(db_path)
    if element_totals_defending:
        create_stacked_bar_graph_defending_only(element_totals_defending, effectiveness_order_defending)
